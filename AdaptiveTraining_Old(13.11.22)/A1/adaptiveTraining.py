...
#def Mental_Work_Load(data): # Get the massages from the Sub from matlab 

#       load = (data.data)
#       if (load == 1.0):
#           print('234999999999999995')
#           A.isRunning=False
           #os.system('kill -9 $(ps -e | grep SimCloud | head -c 6)') 
#           sys.exit(0)
           


class scenarioLaunch:
    def __init__(self, scenarioID="", scriptToRun="", sensorPreset="", queueID="lab_04:0e:3c:33:26:12"):
        #### --- CONSTANTS --- ####
        self.MINTIME = 15
        ...
        #self.sub_mental = rospy.Subscriber("/HR", Float64, self.Mental_Work_Load)
    ...
    def run_alias_thread_func(self):
        ...
        while i < 10: # Runs until the 'break' keyword or after 10 attempts
            ...
            os.system('rosrun cognata_sdk ROSCognataSDK 10.20.1.158 4445 --optimize-rendering --gps > /dev/null')
...
if __name__ == '__main__':
   ...
   # sub_mental = rospy.Subscriber("/HR", Float64, Mental_Work_Load)	
